...
function createGame(topic_id, user1_id, user2_id) {
	$.ajax({
		...
		success: function(data) {
			$.mobile.changePage("views/play.php?game_id=" + data);
		}
	});
}
...
function chooseDifficulty(game_id, user_id, difficulty) {
	$.ajax({
		...
		success: function(data) {
			$.mobile.changePage("views/give_clue.php?game_id=" + game_id + "&answer_id=" + data + "&difficulty=" + difficulty);
		}
	});
}
...
function validateGuess(game_id, clue_id, user_id, points) {
	...
	$.ajax({
		...
		success: function(data) {
			if (data != 0) {
				alert('You got it!');
				$.mobile.changePage("views/got_right.php?game_id=" + game_id + "&answer_id=" + data);
			} else {
				alert('Try again.');
			}
		}
